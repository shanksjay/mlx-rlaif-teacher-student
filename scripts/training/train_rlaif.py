...
_RE_QUANTITY = re.compile(r"\b\d+\s*(items|elements|rows|cols|nodes)\b|\bup to\b")
...
_RE_PERCENT_SCORE = re.compile(r"(?<!\d)(\d+(?:\.\d+)?)\s*%(?!\d)")
_RE_FLOAT_SCORE = re.compile(r"(?<!\d)(?:0(?:\.\d+)?|1(?:\.0+)?|\.\d+)(?!\d)")
...
def _rubric_difficulty_components(prompt: str, language: str) -> dict[str, float]:
    ...
    if has_any(_QUALITY_EXTRA_KEYWORDS):
        quality_hits += 1
...
def _extract_score(text: str) -> Optional[float]:
    ...
    m = _RE_PERCENT_SCORE.search(cleaned)
    if m:
        try:
    ...
    m = _RE_FLOAT_SCORE.search(cleaned)
    if m:
        try:
            v = float(m.group(0))
            if 0.0 <= v <= 1.0:
